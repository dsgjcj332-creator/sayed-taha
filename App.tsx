...
const Gallery3D: React.FC<{ images: string[]; lang: Lang }> = ({ images, lang }) => {
  ...
  return (
    ...
    >
      ...
      <nav className="fixed top-0 w-full z-[100] p-4 md:p-6">
        <motion.div style={{ opacity }} className="max-w-7xl mx-auto glass rounded-[2rem] p-3 md:p-4 flex items-center justify-between shadow-2xl">
        ...
        <AnimatePresence mode="wait">
          {mode === AppMode.VIEW ? (
            <motion.div key="view" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-32 md:space-y-48">
              ...
              {/* 3D Dynamic Gallery */}
              <Gallery3D images={profile.galleryImages} lang={lang} />
            ...
            <motion.div key="edit" initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} className="max-w-6xl mx-auto glass p-6 md:p-12 rounded-[3rem] md:rounded-[4rem] space-y-12 mb-24 shadow-3xl">
